...
#define SERIALIZER_FOR_POD(type)                                               \
  ...
  // Special serializer for floating point values, can't directly swap on
  // floating point variable because it might cause a NaN and the wrong value is
  // read back (the problem occurs on vstudio)
